...
//        console.log("BondToken:", BondTokenAddr);
        ...
        const couponrate = await bondContract.methods.couponRate().call();
        console.log("coupon rate", couponrate.toString());

        const couponinterval = await bondContract.methods.couponInterval().call();
...
*/
      ...
      if ((accounts[0]).toUpperCase() === issuerwalletaddr.toUpperCase()) {
          ...
          this.setState({ ...this.state, 
            ...
            couponToPay: this.state.selectedBondObj.totalsupply * couponAdjustment * (couponrate / 100) * BOND_UNITS,
  ...
  */}
              ...
              <tr><td>S/N</td><td>Investors</td><td>Holdings</td><td>Coupon due</td></tr>
                ...
                {this.state.bondHolders.holders.map((hh, index) => 
                    {
                      return (<tr><td>{index}</td><td>{this.shorten(hh)}</td><td>{Math.floor(this.state.bondHolders.balances[index]/1e18)}</td><td>{(this.state.bondHolders.balances[index]/1e18) * (this.state.couponRate/100) * this.state.couponAdjustment * BOND_UNITS} {this.state.selectedBondObj.campaign.tokenname}</td></tr>)
                    }
                )}
