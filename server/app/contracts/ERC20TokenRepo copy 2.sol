...
contract ERC20TokenRepo {
    ...
    function startTrade(uint256 _tradeId) public onlyAdminOrOwner notAfterEndDate whenNotPaused {
        ...
        if (!successBond) {
            ...
            if (!refundSuccess) {
                // Note: careshTokens are stuck in contract; consider emitting event or handling manually
        ...
        if (!finalCashSuccess) {
            // Note: cashToken are stuck in contract; consider emitting event or handling manually
