...
contract BondToken is ERC20, Ownable {
    ...
    // Use a scaling factor to simulate fixed-point arithmetic (18 decimals)
    ...
    constructor(
        ...
        issuer = _issuer;
                ...
                emit DebugUint("Constructor(): The value of couponRate is:", couponRate);
                emit DebugUint("Constructor(): The value of faceValue is:", faceValue);
                emit DebugUint("Constructor(): The value of couponInterval is:", couponInterval);
                emit DebugUint("Constructor(): The value of maturityDate is:", maturityDate);
                emit DebugAddress("Constructor(): The value of issuer is:", issuer);
                // emit DebugAddress("Constructor(): The value of cashToken is:", cashToken);
        ...
        if (_couponInterval > 0) {
            ...
            nextCouponDate = 0;
                    ...
                    emit DebugUint("Constructor(): The value of nextCouponDate is:", nextCouponDate);

        // Mint initial supply to the contract owner (issuer might be a different address)
        _mint(msg.sender, _totalSupply);

                    emit DebugUint("Constructor(): The value of _totalSupply is:", _totalSupply);
    ...
    function depositCoupons(uint256 amount) external onlyOwnerOrIssuer {
        ...
        require(success, "cashToken transfer failed");
                    ...
                    emit DebugUint("depositCoupons(): The value of amount is:", amount);
                    emit DebugAddress("depositCoupons(): The value of msg.sender is:", msg.sender);
...
//        require(block.timestamp >= nextCouponDate, "Too early for next coupon payment");
        ...
        uint256 couponPerToken = calculateNextCouponPerToken();
                    ...
                    emit DebugUint("payCoupon(): The value of couponPerToken is:", couponPerToken);
        ...
        for (uint256 i = 0; i < holdersLength; i++) {
            ...
            uint256 holderBalance = balanceOf(holder);
                    ...
                    emit DebugAddress("payCoupon(): The value of holder is:", holder);
                    emit DebugUint("payCoupon(): The value of holderBalance is:", holderBalance);

            if (holderBalance > 0) {
                // Since couponPerToken is scaled, divide by SCALE to get the actual amount
                uint256 paymentAmount = (holderBalance * couponPerToken) / SCALE;

                    emit DebugUint("payCoupon(): The value of paymentAmount is:", paymentAmount);
...
*/
        ...
        if (couponInterval > 0) {
            nextCouponDate += couponInterval;
                    ...
                    emit DebugUint("payCoupon(): The value of couponInterval is:", couponInterval);
                    emit DebugUint("payCoupon(): The value of nextCouponDate is:", nextCouponDate);
    ...
    function redeem(uint256 amount) external {
        ...
        _burn(msg.sender, amount);
                ...
                emit DebugAddress("redeem(): The value of msg.sender is:", msg.sender);
                emit DebugUint("redeem(): The value of faceValue is:", faceValue);
                emit DebugUint("redeem(): The value of redemptionAmount is:", redemptionAmount);
                emit DebugUint("redeem(): The value of amount is:", amount);
    ...
    function withdrawSurplus(uint256 amount) external onlyOwner {
        ...
        require(
            ...
            "Not enough cashToken in contract"
                ...
                emit DebugAddress("withdrawSurplus(): The value of owner() is:", owner());
                emit DebugUint("withdrawSurplus(): The value of amount is:", amount);
        ...
        )
    {
        ...
        uint256 couponPerToken = calculateNextCouponPerToken();
                    ...
                    emit DebugUint("bondHolders(): The value of couponPerToken is:", couponPerToken);
        ...
        for (uint256 i = 0; i < length; i++) {
            ...
            nextCoupons[i] = (balanceOfHolder * couponPerToken) / SCALE;
                    ...
                    emit DebugAddress("bondHolders(): The value of holder is:", holder);
                    emit DebugUint("bondHolders(): The value of balanceOfHolder is:", balanceOfHolder);
        }
    }
     ...
     *      The result is scaled by SCALE (i.e. has 18 decimals of precision).
     */
    function calculateNextCouponPerToken() 
        public 
        // view 
        returns (uint256) {
        ...
        // To preserve the fractional part, we multiply by SCALE before dividing.

        uint256 secondsInYear = 365 days;
        uint256 yearlyCouponPerToken = (faceValue * couponRate) / 10000;

                    emit DebugUint("calculateNextCouponPerToken(): The value of faceValue is:", faceValue);
                    emit DebugUint("calculateNextCouponPerToken(): The value of couponRate is:", couponRate);
                    emit DebugUint("calculateNextCouponPerToken(): The value of yearlyCouponPerToken is:", yearlyCouponPerToken);
                    emit DebugUint("calculateNextCouponPerToken(): The value of couponInterval is:", couponInterval);
                    emit DebugUint("calculateNextCouponPerToken(): The value of secondsInYear is:", secondsInYear);

        if (couponInterval == 0) {
            return 0;
        } else {
            // Multiply by SCALE to retain the decimal part before dividing.
            uint256 couponPerToken = (yearlyCouponPerToken * couponInterval * SCALE) / secondsInYear;
    
                    emit DebugUint("calculateNextCouponPerToken(): The value of couponPerToken is:", couponPerToken);
