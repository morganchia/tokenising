...
contract BondToken is ERC20, ERC20Pausable, Ownable {
    ...
    // Events
    event CouponPaid(address indexed to, uint256 couponpaid, uint256 amount, uint256 timestamp, uint256 couponIndex);
    ...
    function payCoupon(uint256 couponIndex, address[] calldata holderList, uint256[] calldata amounts) public onlyOwnerOrAdmin {
        ...
        for (uint256 i = 0; i < holderList.length; i++) {
            ...
            if (balanceOf(holder) > 0 && !_blacklist[holder]) {
                ...
                // Correct formula expectedAmount = (amounts[i] / scalingFactor) * (config.couponRate / 10000) * (couponAdjustment / 12);
                // rearrange to avoid becoming decimals ==> 0
                uint256 expectedAmount = (amounts[i] * config.couponRate * couponAdjustment / 12 / 1000 / scalingFactor);
...
//                require(cashToken.balanceOf(address(this)) >= amounts[i], "Bond: Insufficient cash token balance");
                ...
                // Assuming amounts[i] and expectedAmount are uint256, and cashToken.balanceOf returns uint256
                //string memory errormsg = string(abi.encodePacked("Bond: Invalid coupon amount. Amount=", Strings.toString(amounts[i]), ", Expected=", Strings.toString(expectedAmount), ", balanceOf(holder)=", Strings.toString(balanceOf(holder)), ", couponPerToken=", Strings.toString(couponPerToken), ", scalingFactor=", Strings.toString(scalingFactor)));
                //require(amounts[i] == expectedAmount, errormsg);
                ...
                emit CouponPaid(holder, expectedAmount, amounts[i], block.timestamp, couponIndex);
            }
        }
    }
