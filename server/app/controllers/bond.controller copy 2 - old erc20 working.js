      ...
      async function triggerCoupon() {
        ...
        try {
          const depositCoupons = async () => {
            console.log("Bond smartcontract addr: ", req.body.sender);            
            console.log("Bond ABI: ", ABI);

            const ERC20TokenisedBondcontract = new web3.eth.Contract(ABI, req.body.sender);
...
//              await ERC20TokenisedBondcontract.methods.depositCoupons(couponToPay)
              await ERC20TokenisedBondcontract.methods.payCoupon()
                      ...
                      .catch((error2) => {
                        console.log("Error while estimating Gas fee: ", error2.message);
                        var err;
                        if (error2.message.includes("Insufficient token")) {
                          err = "There is insufficient cash tokens in the Bond smart contract for coupon payment. Issuer needs to fund the operation.";
                        } else if (error2.message.includes("insufficient funds")) {
                          err = "Insufficient funds to transfer tokens.";
                        } else if (error2.message.includes("CONNECTION ERROR")) {
                        ...
                        return -1;  // if error then use default fee
                      });
            ...
            const createTransaction = await web3.eth.accounts.signTransaction(
                  {
                    ...
                    to: req.body.sender,
                    data: contractTx.encodeABI(),
                    gas: Math.floor(gasFees * 2),  // increase by 10% // 8700000,  // 4700000,
                  },
                  signer.privateKey
            );
