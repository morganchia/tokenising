...
//      const ETHEREUM_NETWORK = process.env.REACT_APP_ETHEREUM_NETWORK;
      ...
      async function compileSmartContract() {
        ...
        // Reading the file
        samplefile = fs.readFileSync("./server/app/contracts/ERC20TokenPBM.sample.sol").toString();
        // console.log(file);
        
        //// form the require blocks
        var pbmRedeemConditions = "";

        console.log(">>>> req.body.datafield1_name = ", req.body.datafield1_name);
        console.log(">>>> req.body.datafield1_value = ", req.body.datafield1_value);

        if (req.body.datafield1_name && req.body.datafield1_value) {
          console.log("YAYAYAYA");
          pbmRedeemConditions += 
            "require( \
              ( \
                (\
                  (keccak256(bytes(field1))      == keccak256(bytes(_field1))     ) \
                  && \
                  (keccak256(bytes(field1value)) == keccak256(bytes(_field1value)) ) \
                )\
            ";
        }
        if (req.body.datafield2_name && req.body.datafield2_value) { 
          const __operator = (() => {
            switch (req.body.operator1) {
            case "AND":
              return " && "
            case "OR":
              return " || "
          ...
          pbmRedeemConditions += 
            __operator + " (\
              (keccak256(bytes(field2))      == keccak256(bytes(_field2))     ) \
              && \
              (keccak256(bytes(field2value)) == keccak256(bytes(_field2value)) ) \
            )";
        }
        ...
        if (req.body.datafield1_name && req.body.datafield1_value) {
          pbmRedeemConditions += 
            ") \
            , \"PBM redeem condition failed\");";

          ////
        }

        console.log("pbmRedeemConditions:"+pbmRedeemConditions);
        samplefile = samplefile.replace("\/\/ # PBM redeem conditions #", pbmRedeemConditions); /// replaces comments in .sol file with pbmRedeemConditions

        await fs.writeFile("./server/app/abis/ERC20TokenPBM.sample."+req.body.id+".sol", samplefile , 'utf8', function (err) {
          if (err) {
            console.log("An error occured while writing PBM sol to File.");
            return console.log(err);
          }
          console.log("PBM sol file has been saved.");
        });
        ...
        ABI = output.contracts["ERC20TokenPBM.sample.sol"]["PBMToken"].abi;
        bytecode = output.contracts["ERC20TokenPBM.sample.sol"]["PBMToken"].evm.bytecode.object;
        // console.log("solc.compile output: ", output);
        // console.log("ABI: ", ABI);
        // console.log("Bytecode: ", bytecode);
        await fs.writeFile("./server/app/abis/ERC20TokenPBM.sample."+req.body.id+".abi.json", JSON.stringify(ABI) , 'utf8', function (err) {
          if (err) {
        ...
        await fs.writeFile("./server/app/abis/ERC20TokenPBM.sample."+req.body.id+".bytecode.json", JSON.stringify(bytecode) , 'utf8', function (err) {
          if (err) {
      ...
      async function dAppCreate() {
        ...
        try {
//          if (! (fs.existsSync("./server/app/abis/ERC20TokenPBM.sample.abi.json") && fs.existsSync("./server/app/abis/ERC20TokenPBM.sample.bytecode.json"))) {
...
/*
            ...
            ABI = JSON.parse(fs.readFileSync("./server/app/abis/ERC20TokenPBM.sample.abi.json").toString());
            console.log("Read PBM Bytecode JSON file.");
            bytecode = JSON.parse(fs.readFileSync("./server/app/abis/ERC20TokenPBM.sample.bytecode.json").toString());
          }
*/
        } catch(err) {
...
//        console.log("setToTalSupply = ", setToTalSupply);
        ...
        try {
          ...
          const deployContract = async () => {
            ...
            const ERC20TokenPBM_sample_contract = new web3.eth.Contract(ABI);
            const contractTx = await ERC20TokenPBM_sample_contract.deploy({
              data: bytecode,
          ...
          try {
            ...
            const createTransaction = await web3.eth.accounts.signTransaction(
              { // Sign transaction to setTotalSupply in smart contract
                ...
                data: ERC20TokenPBM_sample_contract.methods.updateTotalSupply(
...
exports.approveWrapMintDraftById = async (req, res) => {  // xxxxx not ready 
  ...
  const amountToSend = new BN(req.body.amount).mul(new BN("1000000000000000000")); 
