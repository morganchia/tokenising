...
const Bridge_Draft = db.bridges_draft;
...
// Create and Save a new Bridge draft
exports.draftCreate = async (req, res) => {
  ...
  console.log("Received for Bridge draft Create:");
  console.log(req.body);

  // Save Bridge draft in the database
  await Bridge_Draft.create(
    { 
      name                  : req.body.name,
      ...
      approvedbridgeid        : req.body.approvedbridgeid,
  ...
  .then(data => {
    try {
      logDataValues("Bridge_draft create: ", data);
    
      // write to audit
      AuditTrail.create(
        { 
          action                : "Bridge draft "+(req.body.txntype===0?"create":req.body.txntype===1?"update":req.body.txntype===2?"delete":"")+" request - created",
          name                  : req.body.name,
          securityname          : req.body.securityname, 
          ISIN                  : req.body.ISIN, 
          tokenname             : req.body.tokenname, 
          ...
          approver              : req.body.approver,
          actionby              : req.body.actionby,
          bridgeid                : req.body.approvedbridgeid,
          status                : 1,   // 0 = draft; 1 = created pending review; 2 = reviewed pending approval; 3 = approved
        }, 
      )
      .then(auditres => {
        console.log("Data written to audittrail for creating draft bridge request.");

      })
      .catch(err => {
        console.log("Error while logging to audittrail for creating draft bridge request: "+err.message);
      });
    } catch (e) {
      console.log("Error:", e);
    }
  ...
  .catch(err => {
    res.status(500).send({
      message:
        err.message || "Some error occurred while creating the Bridge draft."
    });
    console.log("Error while creating bridge draft: "+err.message);
  });
};  // draftCreate
...
exports.create_review = async (req, res) => {
  ...
  console.log("Received for Bridge Review:");
  console.log(req.body);

  await Bridge_Draft.update(
      { 
      ...
      .then(num => {
        if (num == 1) {
          res.send({
            message: "Bridge status has been updated successfully."
          });
        } else {
          res.send({
            message: `${req.body}. Record updated =${num}. Cannot update Bridge with id=${id}. Maybe Bridge was not found or req.body is empty!`
          });
        }
      })
      .catch(err => {
        console.log(err);
        res.status(500).send({
          message: `Error updating Bridge. ${err}`
        });
      }); 
}; // create_review
...
exports.approveDraftById = async (req, res) => {  // 
  // Steps:
  // 1. Is this a new Bridge creation or Edit? If approvedbridgeid === '-1' then it is a new creation
  // 2. If new bridge creation:
  //   a. Check if smart contract is compiled (ABI and ByteCode files are present)
  //   b. Sign smart contract
  //   c. Deploy smart contract
  //   d. Update Bridge_Draft table status to "3"
  //   e. Insert entry in Bridge table
  // 3. If edit existing bridge:
  //   a. Update smart contract info such as total supply or date
  //   b. Update Bridge_Draft table status to "3"
  //   c. Update entry in Bridge table
  ...
  if (req.body.txntype !==0     // create bridge
    && req.body.txntype !==1    // update bridge
    ) {
      if (!errorSent) {
        res.status(400).send({
  ...
  const isNewBridge = (req.body.smartcontractaddress === "" || req.body.smartcontractaddress === null? true : false); // Create = true, Edit/Update = false
...
//      const ETHEREUM_NETWORK = process.env.REACT_APP_ETHEREUM_NETWORK;
      ...
      async function compileSmartContract() {
        ...
        // Reading the file
//        file = fs.readFileSync("./server/app/contracts/ERC20TokenisedBridge.sol").toString();
        file = fs.readFileSync("./server/app/contracts/ERC20Bridge_new.sol").toString();
        // console.log(file);
...
//        ABI = output.contracts["ERC20TokenisedBridge.sol"]["BridgeToken"].abi;
//        bytecode = output.contracts["ERC20TokenisedBridge.sol"]["BridgeToken"].evm.bytecode.object;
        ABI = output.contracts["ERC20Bridge_new.sol"]["BridgeToken"].abi;
        bytecode = output.contracts["ERC20Bridge_new.sol"]["BridgeToken"].evm.bytecode.object;
        // console.log("solc.compile output: ", output);
        // console.log("ABI: ", ABI);
        // console.log("Bytecode: ", bytecode);

                
//        await fs.writeFile("./server/app/abis/ERC20TokenisedBridge.abi.json", JSON.stringify(ABI) , 'utf8', function (err) {
        await fs.writeFile("./server/app/abis/ERC20Bridge_new.abi.json", JSON.stringify(ABI) , 'utf8', function (err) {
          if (err) {
            console.log("An error occured while writing Bridge ABI JSON Object to File.");
            return console.log(err);
          }
          console.log("Bridge ABI JSON file has been saved.");
        });
//         await fs.writeFile("./server/app/abis/ERC20TokenisedBridge.bytecode.json", JSON.stringify(bytecode) , 'utf8', function (err) {
        await fs.writeFile("./server/app/abis/ERC20Bridge_new.bytecode.json", JSON.stringify(bytecode) , 'utf8', function (err) {
          if (err) {
            console.log("An error occured while writing Bridge bytecode JSON Object to File.");
            return console.log(err);
          }
          console.log("Bridge Bytecode JSON file has been saved.");
        });
      ...
      async function dAppCreate() {
        ...
        try {
//          if (! (fs.existsSync("./server/app/abis/ERC20TokenisedBridge.abi.json") && fs.existsSync("./server/app/abis/ERC20TokenisedBridge.bytecode.json"))) {
            await compileSmartContract();
/*
          } else{
            // Just read the ABI file
            console.log("Bridge ABI and Bytecode files are present, just read them, no need to recompile...");
            console.log("Read Bridge ABI JSON file.");
            ABI = JSON.parse(fs.readFileSync("./server/app/abis/ERC20TokenisedBridge.abi.json").toString());
            console.log("Read Bridge Bytecode JSON file.");
            bytecode = JSON.parse(fs.readFileSync("./server/app/abis/ERC20TokenisedBridge.bytecode.json").toString());
          }
*/
        } catch(err) {
          ...
          if (!errorSent) {
            ...
            res.status(400).send({ 
              message: "Error when compiling Bridge smart contract. Please contact your tech support."
            });
            errorSent = true;
          }
          return false;
...
//        console.log("setToTalSupply = ", setToTalSupply);
        ...
        try {
          ...
          const deployContract = async () => {
              ...
              const ERC20TokenisedBridgecontract = new web3.eth.Contract(ABI);
              ...
              // Structure BridgeConfig as an array
              const bridgeConfig = [
              ...
              // Estimate gas fee
              const gasFees = await retryWithBackoff( () => ERC20TokenisedBridgecontract.deploy({
                  data: bytecode,
                  arguments: [
                      req.body.tokenname,
              ...
              const contractTx = await retryWithBackoff( () => ERC20TokenisedBridgecontract.deploy({
                  data: bytecode,
                  arguments: [
                      req.body.tokenname,
                      req.body.tokensymbol,
                              ...
                              web3.eth.getTransactionReceipt(hash, async function (error3, receipt) {
                                  ...
                                  if (timer > TIMEOUT) {
                                      ...
                                      if (!errorSent) {
                                          console.log("Sending error 400 back to client");
                                          res.status(400).send({
                                              message: "Timeout after " + TIMEOUT.toString() + " seconds, please check the Bridge tab after 5 minutes and try again if the Bridge isnt created.",
                                          });
                                          errorSent = true;
                                      }
                                      return false;
      ...
      async function dAppUpdate() {
        ...
        fs = require("fs");
//        ABI = JSON.parse(fs.readFileSync("./server/app/abis/ERC20TokenisedBridge.abi.json").toString());
        ABI = JSON.parse(fs.readFileSync("./server/app/abis/ERC20Bridge_new.abi.json").toString());
        ...
        const UpdateContract = async () => {
          try {
            console.log('Creating Bridge contract with ABI');
            const ERC20TokenisedBridgecontract = new web3.eth.Contract(ABI);
            ...
            + createStringWithZeros(adjustdecimals);  // pad zeros behind
            console.log("Bridge setToTalSupply = ", setToTalSupply);
            ...
            const createTransaction = await web3.eth.accounts.signTransaction(  
              { // Sign transaction to setTotalSupply in smart contract
                ...
                data: ERC20TokenisedBridgecontract.methods.updateTotalSupply(
            ...
            console.log('**** Bridge Txn executed:', createReceipt);
            return true;
          } catch(error) {
      ...
      /*
      console.log("*** isNewBridge = ", isNewBridge);
...
////////////////////////////// Blockchain ////////////////////////
  ...
  console.log('New Bridge Contract deployed updating DB: ', newcontractaddress);

  if (updatestatus) {
  // update draft table
    await Bridge_Draft.update(  // update draft table status to "3"
    { 
      status                : 3,
    ...
    if (isNewBridge) {
      await Bridge.create( // create Bridge in the database !!!!!
        { 
          name                  : req.body.name,
          securityname          : req.body.securityname,
          ISIN                  : req.body.ISIN, 
...
//          datafield2_value      : req.body.datafield2_value,
          ...
          actionby              : req.body.actionby,
          draftbridgeid           : req.body.id,          
        }, 
      )
      .then(data => {
        logDataValues("Bridge create success: ", data);
      ...
      .catch(err => {
        console.log("Error while creating bridge: "+err.message);
        if (!errorSent) {
          console.log("Sending error 400 back to client");
          res.status(400).send({ 
            message: 'Error when signing transaction. Please try again. Report to tech support if problem is recurring.',
    ...
    } else { // not isNewBridge
      await Bridge.update( // update Bridge in the database !!!!! 
      { 
        name                  : req.body.name,
        securityname          : req.body.securityname,
        ISIN                  : req.body.ISIN, 
...
//        datafield2_value      : req.body.datafield2_value,
      ...
      { where:      { id: req.body.approvedbridgeid }},
      )
      .then(data => {
        logDataValues("Bridge update success: ", data);
      ...
      .catch(err => {
        console.log("Error while updating bridge: "+err.message);
        if (!errorSent) {
          console.log("Sending error 400 back to client");
          res.status(400).send({ 
            message: 'Error when signing transaction. Please try again. Report to tech support if problem is recurring.',
...
exports.triggerBridgeCouponPaymentById = async (req, res) => {
  let errorSent = false;
  let updatestatus = false;

  const bridge_id = req.params.id;
  const { lowestUnpaidCouponIndex, holders, amountsHeld } = req.body;

  console.log("Received triggerBridgeCouponPaymentById:");
  //console.log(req.body);
  console.log("triggerBridgeCouponPaymentById called with bridge_id:", bridge_id);
...
/*
  // Validate request
  if (bridge_id === undefined || typeof bridge_id !== "number" || bridge_id <= 0 || typeof lowestUnpaidCouponIndex !== "number" || holders === undefined || !Array.isArray(holders) || holders.length === 0 || amountsHeld === undefined || !Array.isArray(amountsHeld) || amountsHeld.length === 0) {
    if (!errorSent) {
      res.status(400).send({
        message: "Invalid request: Missing bridge ID, coupon index, holder list, or amounts held."
      });
      errorSent = true;
    }
    return;
...
*/
  ...
  let bridge;
  try {
    bridge = await Bridge.findByPk(bridge_id);
    if (!bridge) {
      if (!errorSent) {
        res.status(404).send({
          message: `Bridge with id=${bridge_id} not found.`
        });
        errorSent = true;
      }
      return;
    ...
    console.error("Error fetching bridge:", err.message);
    if (!errorSent) {
      res.status(500).send({
        message: "Error fetching bridge details."
      });
      errorSent = true;
    }
    return;
  ...
  require('dotenv').config();
  const ETHEREUM_NETWORK = (() => {
    switch (Number(bridge.blockchain)) {
  ...
  async function triggerCoupon() {
    ...
    const fs = require("fs");
    try {
      console.log("Reading Bridge ABI JSON file.");
      ABI = JSON.parse(fs.readFileSync("./server/app/abis/ERC20Bridge_new.abi.json").toString());
    } catch (err) {
      console.error("Err reading ABI:", err);
      if (!errorSent) {
        res.status(400).send({
          message: "Error reading bridge smart contract ABI."
        });
        errorSent = true;
      }
      return false;
    ...
    try {
      const payCoupon = async () => {
        console.log("Coupon Payment Inputs:", {
          ...
          //faceValue,
          //couponRate,
          //holderBalance: await bridgeContract.methods.balanceOf(holders[0]).call()
        });

        console.log("Bridge smart contract address:", bridge.smartcontractaddress);
        const bridgeContract = new web3.eth.Contract(ABI, bridge.smartcontractaddress);
        ...
        /*
        try {
          gasFees = await bridgeContract.methods.payCoupon(lowestUnpaidCouponIndex, holders, amountsHeld)
          ...
          if (error.message.includes("Coupon already paid")) {
            ...
            errMessage = "Insufficient cash tokens in the bridge contract for coupon payment.";
          } else {
        ...
        */
        // Prepare transaction
        const contractTx = bridgeContract.methods.payCoupon(lowestUnpaidCouponIndex, holders, amountsHeld);
        const createTransaction = await web3.eth.accounts.signTransaction(
          {
            nonce: nonce,
            from: signer.address,
            to: bridge.smartcontractaddress,
            data: contractTx.encodeABI(),
  ...
  console.log("triggerBridgeCouponPaymentById Update status:", updatestatus);

  // Log to audit trail if successful
  if (updatestatus) {
    try {
      await AuditTrail.create({
        action: "Bridge coupon payment",
        name: bridge.name,
        securityname: bridge.securityname,
        ISIN: bridge.ISIN,
        tokenname: bridge.tokenname?.toUpperCase(),
        tokensymbol: bridge.tokensymbol,
        blockchain: bridge.blockchain,
        facevalue: bridge.facevalue,
        couponrate: bridge.couponrate,
        couponinterval: bridge.couponinterval,
        issuer: bridge.issuer,
        totalsupply: bridge.totalsupply,
        prospectusurl: bridge.prospectusurl,
        smartcontractaddress: bridge.smartcontractaddress,
        cashTokenID: bridge.cashTokenID,
        CashTokensmartcontractaddress: bridge.CashTokensmartcontractaddress,
        couponIndex: lowestUnpaidCouponIndex
      });
      console.log("Audit trail logged for bridge coupon payment.");
    } catch (err) {
      console.error("Error logging to audit trail:", err.message);
    }
  }
}; // triggerBridgeCouponPaymentById
...
exports.findDraftByNameExact = (req, res) => {
  ...
  Bridge_Draft.findAll(
    { where: condition },
    )
    .then(data => {
      logDataValues("Bridge_Draft.findAll: ", data);
      res.send(data);
    })
    .catch(err => {
      console.log("Error while retreiving bridge1 draft: "+err.message);

      res.status(400).send({
        message:
          err.message || "Some error occurred while retrieving bridge draft."
      });
    });
...
exports.findExact = (req, res) => {
  ...
  Bridge.findAll(
    ...
    .then(data => {
      logDataValues("Bridge.findAll: ", data);
      res.send(data);
    })
    .catch(err => {
      console.log("Error while retreiving bridge1: "+err.message);

      res.status(500).send({
        message:
          err.message || "Some error occurred while retrieving bridge."
      });
    });
}; // findExact
...
exports.getInWalletMintedTotalSupply = (req, res) => {
  ...
  // get token address from BridgeId
  const Id = req.query.id;
  ...
  .then(async data => {
    ...
    // Readng ABI from JSON file
    fs = require("fs");
//     ABI = JSON.parse(fs.readFileSync("./server/app/abis/ERC20TokenisedBridge.abi.json").toString());  // <-- dropdown menu
    ABI = JSON.parse(fs.readFileSync("./server/app/abis/ERC20Bridge_new.abi.json").toString());  // <-- dropdown menu

    // Creation of Web3 class
    Web3 = require("web3");

    logDataValues("In Bridge.findAll: ", data);
  ...
  .catch(err => {
    console.log("Error while retreiving bridge2: "+err.message);
...
// Retrieve all Bridge from the database.
exports.findByName = (req, res) => {
  ...
  Bridge.findAll(
    ...
    .then(data => {
      logDataValues("Bridge.findByName: ", data);
      res.send(data);
    })
    .catch(err => {
      console.log("Error while retreiving bridge3: "+err.message);
...
exports.getAllByBridgeId = (req, res) => {
  const id = req.query.id;
  console.log("====== bridge.getAllByBridgeId(id) ",id);
  var condition = id ? 
       {id : id}
      : null;
  ...
  Bridge.findAll(
      ...
      include: [
        {
          model: db.recipients,
          on: {
            id: db.Sequelize.where(db.Sequelize.col("bridges.issuer"), "=", db.Sequelize.col("recipient.id")),
          },
          attributes: ['id','name'],
        },
        {
          model: db.campaigns,
          on: {
            id: db.Sequelize.where(db.Sequelize.col("bridges.cashTokenID"), "=", db.Sequelize.col("campaign.id")),
          },
    ...
    .catch(err => {
      console.log("Error while retreiving bridge5a: "+err.message);
    ...
    include: [
      {
        model: db.recipients,
        on: {
          id: db.Sequelize.where(db.Sequelize.col("bridge.issuer"), "=", db.Sequelize.col("recipient.id")),
        },
      ...
      },
      {
        model: db.campaigns,
        on: {
          id: db.Sequelize.where(db.Sequelize.col("bridge.cashTokenID"), "=", db.Sequelize.col("campaign.id")),
        },
        attributes: ['id', 'name', 'tokenname', 'smartcontractaddress','blockchain'],
      }
    ]
...
exports.getAllDraftsByUserId = (req, res) => {
  const id = req.query.id;
  console.log("====== bridge.getAllDraftsByUserId(id) ",id);
  var condition = id ? { 
        [Op.or]: 
        [
          { 
  ...
  Bridge_Draft.findAll( 
      ...
      include: [
        {
          model: db.recipients,
          on: {
            id: db.Sequelize.where(db.Sequelize.col("bridges_draft.issuer"), "=", db.Sequelize.col("recipient.id")),
          },
          attributes: ['id','name'],
        },
        {
          model: db.campaigns,
          on: {
            id: db.Sequelize.where(db.Sequelize.col("bridges_draft.cashTokenID"), "=", db.Sequelize.col("campaign.id")),
          },
          attributes: ['id', 'name', 'tokenname', 'smartcontractaddress','blockchain'],
        }
      ]
    ...
    .catch(err => {
      console.log("Error while retreiving bridge6: "+err.message);
      res.status(500).send({
        message:
          err.message || "Some error occurred while retrieving bridge."
      });
...
exports.getAllDraftsByBridgeId = (req, res) => {
  const id = req.query.id;
  console.log("====== bridge.getAllDraftsByBridgeId(id) ",id);
  var condition = id ? 
       {id : id}
      : null;
...
*/
    ...
    .catch(err => {
      console.log("Error while retreiving bridge5: "+err.message);
...
/*
  ...
  */
}; // getAllDraftsByBridgeId

// Find a single Bridge with an id
exports.findOne = (req, res) => {
  ...
  Bridge.findByPk(id, {
    ...
    .then(data => {
      if (data) {
        logDataValues("Bridge.findByPk: ", data);
        res.send(data);
      } else {
        res.status(404).send({ 
          message: `Cannot find Bridge with id=${id}.`
        });
      }
    })
    .catch(err => {
      res.status(500).send({
        message: "Error retrieving Bridge with id=" + id
      });
    });
}; // findOne
...
exports.getAllInvestorsById = (req, res) => {
  const id = req.query.id;
  console.log("In Bridge.getAllInvestorsById: id=", id);
  let errorSent = false;
  ...
  .then(async data => {
    if (!data) {
      res.status(404).send({ message: `Bridge with id=${id} not found.` });
      return;
    }
    ...
    try {
      ...
      res.status(500).send({ message: "Error reading bridge contract ABI." });
      return;
    }
    ...
    const bridgeTokenAddr = data.smartcontractaddress;
    const bridgeContract = new web3.eth.Contract(ABI, bridgeTokenAddr);
    console.log("Querying token:", bridgeTokenAddr);
    ...
    try {
      // Fetch bridge config and deployment block
      const fetchConfigAndBlock = async () => {
        try {
          ...
          const config = await retryWithBackoff(() => bridgeContract.methods.config().call());
          ...
          const couponCount = Number(await retryWithBackoff(() => bridgeContract.methods.couponCount().call()));
          ...
          for (let i = 0; i < couponCount && currentCouponDate <= maturityDate; i++) {
            ...
            const isPaid = await retryWithBackoff(() => bridgeContract.methods.isCouponPaid(i).call());
          ...
          const fetchString = `https://api${ETHEREUM_NETWORK === 'mainnet' ? '' : '-' + ETHEREUM_NETWORK}.etherscan.io/api` +
            `?module=account&action=txlist&address=${bridgeTokenAddr}&startblock=0&sort=asc&apikey=${ETHERSCAN_API_KEY}`;
      ...
      for (let i = fromBlock; i <= blockNumber; i += step) {
        ...
        const events = await retryWithBackoff( () => bridgeContract.getPastEvents('Transfer', { fromBlock: i, toBlock }),
          3,
          1000,
      ...
      for (const [address, balance] of Object.entries(holders)) {
        if (!balance.isZero()) {
          const isBlacklisted = await retryWithBackoff(() => bridgeContract.methods.isBlacklisted(address).call());
          if (!isBlacklisted) {
            const currentBalance = await retryWithBackoff(() => bridgeContract.methods.balanceOf(address).call());
      ...
      if (!errorSent) {
        res.status(400).send({ message: "Error retrieving bridge holders. Please try again." });
        errorSent = true;
      }
    }
  })
  .catch(err => {
    console.error("Error retrieving bridge:", err.message);
    if (!errorSent) {
      res.status(500).send({ message: "Error retrieving bridge data." });
    }
  });
}; // getAllInvestorsById
...
//    datafield2_value      : req.body.datafield2_value,
  ...
  .then(num => {
    if (num == 1) {
      ...
      AuditTrail.create(
        { 
          action                : "Bridge "+(req.body.txntype===0?"create":req.body.txntype===1?"update":req.body.txntype===2?"delete":"")+" request - resubmitted",
          name                  : req.body.name,
          securityname          : req.body.securityname,
          ISIN                  : req.body.ISIN, 
          tokenname             : req.body.tokenname.toUpperCase(), 
...
//          datafield2_value      : req.body.datafield2_value,
      ...
      .then(auditres => {
        console.log("Data written to audittrail for resubmitting bridge request:", auditres);

      })
      .catch(err => {
        console.log("Error while logging to audittrail for resubmitting bridge request: "+err.message);
      });
      
      res.send({
        message: "Bridge resubmitted successfully."
      });
    } else {
      res.send({
        message: `${req.body}. Record updated =${num}. Cannot update Bridge with id=${draft_id}. Maybe Bridge was not found or req.body is empty!`
      });
    }
  })
  .catch(err => {
...
exports.acceptDraftById = async (req, res) => {
  ...
  .then(num => {
    if (num == 1) {
      ...
      AuditTrail.create(
        { 
          action                : "Bridge "+(req.body.txntype===0?"create":req.body.txntype===1?"update":req.body.txntype===2?"delete":"")+" request - accepted",
          name                  : req.body.name,
          securityname          : req.body.securityname,
          ISIN                  : req.body.ISIN, 
          tokenname             : req.body.tokenname.toUpperCase(), 
...
//          datafield2_value      : req.body.datafield2_value,
      ...
      .then(auditres => {
        console.log("Data written to audittrail for accepting bridge request:", auditres);

      })
      .catch(err => {
        console.log("Error while logging to audittrail for accepting bridge request: "+err.message);
      });
      
      res.send({
        message: "Bridge was accepted successfully."
      });
    } else {
      res.send({
        message: `${req.body}. Record updated =${num}. Cannot update Bridge with id=${draft_id}. Maybe Bridge was not found or req.body is empty!`
...
exports.rejectDraftById = async (req, res) => {
  ...
  .then(num => {
    if (num == 1) {
      ...
      AuditTrail.create(
        { 
          action                : "Bridge "+(req.body.txntype===0?"create":req.body.txntype===1?"update":req.body.txntype===2?"delete":"")+" request - rejected",
          name                  : req.body.name,
          securityname          : req.body.securityname,
          ISIN                  : req.body.ISIN, 
          tokenname             : req.body.tokenname.toUpperCase(), 
...
//          datafield2_value      : req.body.datafield2_value,
      ...
      .then(auditres => {
        console.log("Data written to audittrail for rejecting bridge request:", auditres);

      })
      .catch(err => {
        console.log("Error while logging to audittrail for rejecting bridge request: "+err.message);
      });
      
      if (!errorSent) {
          res.send({
          message: "Bridge was rejected."
        });
        errorSent = true;
      }
    } else {
      if (!errorSent) {
        res.send({
          message: `${req.body}. Record updated =${num}. Cannot reject Bridge with id=${draft_id}. Maybe Bridge was not found or req.body is empty!`
        });
        errorSent = true;
      }
    }
  ...
  .catch(err => {
    ...
    if (!errorSent) {
      res.status(500).send({
        message: `Error rejecting Bridge. ${err}`
      });
      errorSent = true;
    }
  });
...
// Update a Bridge by the id in the request
              ...
              const interval = setInterval(function() {
                ...
                web3.eth.getTransactionReceipt(hash, async function(error3, receipt) {
                  ...
                  if (timer > TIMEOUT) {
                    ...
                    if (!errorSent) {
                      res.status(400).send({ 
                        message: "Timeout after "+TIMEOUT.toString()+" seconds, please check the Bridge tab after 5 minutes and try again if the Bridge is not created.",
                      });
                      errorSent = true;
                    }
                    return false;
...
//        datafield2_value      : req.body.datafield2_value,
      ...
      .then(num => {
        if (num == 1) {
          ...
          AuditTrail.create(
            { 
              action                : "Bridge "+(req.body.txntype===0?"create":req.body.txntype===1?"update":req.body.txntype===2?"delete":"")+" update request - approved",
              name                  : req.body.name,
              securityname          : req.body.securityname, 
              ISIN                  : req.body.ISIN, 
              tokenname             : req.body.tokenname, 
...
//              datafield2_value      : req.body.datafield2_value,
          ...
          .then(auditres => {
            console.log("Data written to audittrail for approving bridge update request:", auditres);

          })
          .catch(err => {
            console.log("Error while logging to audittrail for approving bridge update request: "+err.message);
          });

          if (!errorSent) {
            res.send({
              message: "Bridge was updated successfully."
            });
            errorSent = true;
          }
        } else {
...
// Delete a Bridge with the specified id in the request
exports.approveDeleteDraftById = async (req, res) => {
  ...
  // update draft table
  var Done = await Bridge_Draft.update(  // update draft table status to "3"
  { 
    status            : 3,
    approverComments  : req.body.approvercomments,
  }, 
  ...
  .then(num => {
    if (num == 1) {
      ...
      AuditTrail.create(
        { 
          action                : "Bridge "+(req.body.txntype===0?"create":req.body.txntype===1?"update":req.body.txntype===2?"delete":"")+" request - deleted",
          name                  : req.body.name,
          securityname          : req.body.securityname, 
          ISIN                  : req.body.ISIN, 
          tokenname             : req.body.tokenname, 
...
//          datafield2_value      : req.body.datafield2_value,
      ...
      .then(auditres => {
        console.log("Data written to audittrail for bridge delete request:", auditres);

      })
      .catch(err => {
        console.log("Error while logging to audittrail for bridge delete request: "+err.message);
      });
  ...
  if (Done) await Bridge.destroy({ // delete entry in approved Bridge table
    where: { id: req.body.approvedbridgeid }
  })
  .then(num => {
    if (num == 1) {
      if (!msgSent) {
        console.log("Sending success bridge delete to client");
        res.send({
          message: "Bridge was deleted successfully!"
        });
        msgSent = true;
      }
      return true;
      ...
      if (!msgSent) {
        res.send({
          message: `Cannot delete Bridge with id=${req.body.approvedbridgeid}. Maybe Bridge was not found!`
        });
        msgSent = true;
      }
      return true;
  ...
  .catch(err => {
    if (!msgSent) {
      console.log("Sending error 400 back to client");
      res.status(400).send({ 
        message: 'Error when deleting Bridge from database, please inform tech support.',
      });
      msgSent = true;
    }
    return false;
...
exports.dropRequestById = async (req, res) => {
  ...
  // update draft table
  await Bridge_Draft.update(  // update draft table status to "9" - aborted / dropped requests
  { 
    status            : 9,
    approverComments  : req.body.approvercomments,
  }, 
  ...
  .then(num => {
    if (num == 1) {
      ...
      AuditTrail.create(
        { 
          action                : "Bridge "+(req.body.txntype===0?"create":req.body.txntype===1?"update":req.body.txntype===2?"delete":"")+" request - dropped",
          name                  : req.body.name,
          securityname          : req.body.securityname, 
          ISIN                  : req.body.ISIN, 
          tokenname             : req.body.tokenname, 
...
//          datafield2_value      : req.body.datafield2_value,
      ...
      .then(auditres => {
        console.log("Data written to audittrail for dropping bridge request:", auditres);

      })
      .catch(err => {
        console.log("Error while logging to audittrail for dropping bridge request: "+err.message);
      });
      
      if (!msgSent) {
        console.log("Sending success bridge request dropped to client");
        res.send({
  ...
  Bridge.destroy({
    ...
    .then(num => {
      if (num == 1) {
        ...
        if (!errorSent) {
          res.send({
            message: `Cannot delete Bridge with id=${id}. Maybe Bridge was not found!`
          });
          errorSent = true;
        }
      }
    ...
    .catch(err => {
      if (!errorSent) {
        res.status(500).send({
          message: "Could not delete Bridge with id=" + id
        });
        errorSent = true;
      }
    });
...
// Delete all Bridge from the database.
exports.deleteAll = (req, res) => {
  ...
  Bridge.destroy({
    ...
    .then(nums => {
      res.send({ message: `${nums} Bridge were deleted successfully!` });
    })
    .catch(err => {
      if (!errorSent) {
        res.status(500).send({
          message:
            err.message || "Some error occurred while removing all bridge."
        });
        errorSent = true;
      }
    });
