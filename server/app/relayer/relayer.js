...
try {
  bridgeSourceAbi = require('../abis/BridgeSource.json').abi;
  bridgeDestAbi = require('../abis/BridgeDestination.json').abi;
  wrappedAbi = require('../abis/WrappedCBDC.json').abi;
} catch (err) {
  ...
  bridgeDest.events.Burned({ fromBlock: 'latest' })
    .on('data', async (event) => {
      ...
      try {
        ...
        unlockTx = await withGasPriceRetry(amoyWeb3, async (priorityFee) => {
        ...
        console.log(`Unlocked ${amoyWeb3.utils.fromWei(amount, 'ether')} CBDC to ${destinationAddress} on Amoy. TxHash: ${unlockTx.transactionHash}`);

        // Step 2: Check stagedBurns and confirm burn on Sepolia
        let stagedAmount;
        try {
          stagedAmount = await bridgeDest.methods.stagedBurns(user, nonce).call();
          console.log(`Staged burn amount for user=${user}, nonce=${nonce}: ${sepoliaWeb3.utils.fromWei(stagedAmount, 'ether')} wrCBDC`);
        } catch (err) {
          console.error(`Error checking stagedBurns: ${err.message}`);
          stagedAmount = '0';
        }

        if (BigInt(stagedAmount) < BigInt(amount)) {
          console.warn(`Skipping confirmBurn: Insufficient staged burn amount (${sepoliaWeb3.utils.fromWei(stagedAmount, 'ether')} < ${sepoliaWeb3.utils.fromWei(amount, 'ether')}) for user=${user}, nonce=${nonce}`);
          processedTxs.add(txHash);
          saveProcessedTxs();
          return;
        }
        ...
        console.log(`Confirmed burn for ${sepoliaWeb3.utils.fromWei(amount, 'ether')} wrCBDC for ${user} on Sepolia. TxHash: ${confirmBurnTx.transactionHash}`);
        processedTxs.add(txHash);
        saveProcessedTxs();
      } catch (err) {
        ...
        // If confirmBurn failed but unlock succeeded, still mark as processed
        if (err.message.includes('Burn failed') && unlockTx) {
          console.warn(`confirmBurn failed, but unlock succeeded. Marking txHash=${txHash} as processed.`);
          processedTxs.add(txHash);
          saveProcessedTxs();
        }
      }
